...
class Battle:
    ...
    def play_turn(self, user: int, user_input: int) -> None:
        ...
        self.anim_manager.add_animation(animation.ListenableTextDisplay([pokemon_user.name + " Utilise ", move.name]))
        ...
        if self.player_trainer.get_current_pokemon().is_fainted():
            # FAINTING ANIMATION
            someone_alive = False
            for i in range(0, self.player_trainer.get_pokemon_list_size()):
                if not self.player_trainer.pokemon_list[i].is_fainted():
                    someone_alive = True
                    break
            if someone_alive:
                self.anim_manager.add_animation(
                    animation.ListenableTextDisplay([self.player_trainer.get_current_pokemon().get_name(), " est K.O!"]))
