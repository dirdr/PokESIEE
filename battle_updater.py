...
class BattleUpdater:
    ...
    def listen_option(self):
        if self.user_input == 0:
            ...
            self.queue.clear()
            first_message = "vos pokémon sont :"
            second_message = self.battle.player_trainer.get_pokemon_string()
            anim = animation.ListenableTextDisplay([first_message, second_message])
            anim.set_font_size(10)
            self.anim_manager.add_animation(anim)
            self.restart_turn()
